...
public record SchemaField(
    ...
    public SchemaField {
        ...
        if (type == FieldType.SELECT && choices.isEmpty() && format == FieldFormat.NONE) {
            // 允许空 choices（运行时再填）；不强制
        }
    }

    public SchemaField(String name, FieldType type, boolean required, String description) {
        this(name, type, required, description, name, null, false, List.of(), FieldFormat.NONE);
    }
